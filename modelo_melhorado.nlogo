...
to setUpPatches
  ...
  ask patches [
    set pcolor black
    if random 101 < spawn-prob-food [
      ask one-of patches with [pcolor = black] [
        set pcolor green
      ]
    ]

    if random 101 < spawn-prob-waste [
      ask one-of patches with [pcolor = black] [
        set pcolor yellow
      ]
    ]

    if random 101 < spawn-prob-toxic-waste [
      ask one-of patches with [pcolor = black] [
        set pcolor red
      ]
    ]
...
to setUpTurtles
  ; region turtles
  ;
  create-eaters start-eaters
  create-cleaners start-cleaners
...
to eaterReproduce
  ask eaters [
    if count eaters-here >= 2 and random 101 < reproduce-prob [
      if energy > reproduce-cost [
...
to cleanerRules
  ask cleaners [
    if [pcolor] of patch-here = green [
      ifelse cleaner-waste < cleaner-storage / 2 [
        set energy energy + energy-from-food
      ] [
        set energy energy + energy-from-food / 2
      ]
    ...
    if cleaner-waste < cleaner-storage [
      if [pcolor] of patch-here = yellow [
        set cleaner-waste cleaner-waste + 1
        set pcolor black
        ask one-of patches with [pcolor = black] [
          set pcolor yellow
        ]
      ...
      if [pcolor] of patch-here = red [
        set cleaner-waste cleaner-waste + 2
        set pcolor black
        ask one-of patches with [pcolor = black] [
          set pcolor red
        ]
...
to cleanerActions
  ;waste
    ask cleaners [
    ifelse cleaner-waste < cleaner-storage [
      if [pcolor] of patch-ahead 1 = yellow [
        fd 1
        if cleaner-waste < cleaner-storage [
          set cleaner-waste cleaner-waste + 1
...
"" ""
PENS
"Eaters" 1.0 0 -13840069 true "" "plot count eaters"
...
"" ""
PENS
"Ratio" 1.0 0 -2674135 true "" "plot count eaters / count cleaners"
...
<experiments>
  ...
  <experiment name="trashcans_and_eaters_survival" repetitions="10" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <metric>count eaters</metric>
    <enumeratedValueSet variable="reproduce-cost">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="energy-from-food">
      <value value="50"/>
    ...
    <enumeratedValueSet variable="reproduce-prob">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="start-cleaners">
      <value value="50"/>
